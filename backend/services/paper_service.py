...
def build_paper_bundle_from_df(df: pd.DataFrame, col) -> io.BytesIO:
    ...
    for index, row in df.iterrows():
        part_val = clean_val(row.get('part', 'A'))
        if not part_val: part_val = 'A'
        
        if current_part != part_val:
            ...
            for doc in [qp_doc, ak_doc]:
                ...
                run_p = pp.add_run(f"PART — {str(current_part).upper()}")
                run_p.bold = True
        ...
        q_type = clean_val(row.get('q_type', 'subjective'))
        question_data = None
        
        if q_type in ['mcq_general', 'mcq_scenario']:
            import requests
            # 1. Fetch base context/knowledge from the external endpoint matching this exact level/chapter
            try:
                # Based on the user's endpoint format, adapt this URL if necessary to fetch the scenario
                url = f"http://31.97.228.184:5555/api/data/questions?level={level}&subject={subject}&chapter={chapter}"
                # Here you would perform the request:
                # res = requests.get(url)
                # context_text = res.json()
                
                context_text = "PLACEHOLDER KNOWLEDGE FETCHED FROM EXTERNAL ENDPOINT"
                
                # 2. Add your LLM Connection here:
                # example: llm_res = openai.ChatCompletion.create(messages=[{"role":"user", "content": f"Create {q_type} MCQ from {context_text}"}])
                # Let's say the LLM returns JSON with query options
                
                question_data = {
                    "question_text": f"[AI GENERATED MCQ based on {q_type.upper()}]\nIdentify the appropriate statement regarding {subject} in {chapter}.\n\nA) Option 1\nB) Option 2\nC) Option 3\nD) Option 4",
                    "answer_text": "Correct Answer: B\nExplanation: This is auto-generated by your LLM using the fetched external endpoint context."
                }
            except Exception as e:
                logger.error(f"Error fetching/generating MCQ for {q_num}: {e}")
        else:
            # --- Standard Workflow ---
            if unit:
